...
(function(){
  ...
  // === RESCATE: estado y banderas ===
  let gameMode = "skirmish";       // "skirmish" (nivel actual) -> "rescue"
  let nextMode = null;             // decide qué hace el botón Continuar al ganar
  let aldeano = null;
  ...
  // === RESCATE: unidad Aldeano ===
  const makeVillager = () => ({
    id: "V", tipo: "aldeano",
    fila: Math.floor(ROWS*0.75), col: Math.floor(COLS*0.18),
    vivo: true, nombre: "Aldeano",
    hp: 60, maxHp: 60,
    retrato: "assets/villager.png", // si no tienes sprite, usa temporalmente assets/archer.png
  ...
  function spawnFase(){
    // si estamos en RESCATE, no usar este spawner
  ...
  function spawnRescue(){
    ...
    for (let i=0;i<count;i++){
      let f,c;
      do{
        f = Math.floor(Math.random()*(ROWS - NON_PLAYABLE_BOTTOM_ROWS - 3)); // algo más arriba
        c = Math.floor(Math.random()*COLS);
  ...
  function manejarClick(f,c){
    ...
    if (seleccionado){
      ...
      if (esAlcanzable && !ocupado){
        ...
        // === RESCATE: victoria por llegada a salida
  ...
  function aplicarDanyo(obj,cant,fuente){
    ...
    if(obj.hp<=0){
      obj.vivo=false;
      efectoMuerte(obj);
      // === RESCATE: si muere el aldeano → derrota
  ...
  function turnoIAEnemigos(){
    ...
    // objetivo más cercano; en RESCATE prioriza aldeano si vive
    ...
    for (const en of enemies) {
      ...
      // objetivo más cercano por distancia Manhattan
      ...
      // moverse hacia objetivo evitando choques y zona no jugable
    ...
    if (controlables.every(p=>!p.vivo)) { setTurno("fin"); }
      ...
      setTurno("jugador");
      // en skirmish mantenemos la lógica de oleadas original
      if (gameMode==="skirmish" && enemies.every(e=>!e.vivo)) {
        if (fase === 1){ fase = 2; spawnFase(); dibujarMapa(); }
        else if (fase === 2){
          fase = 3;
          // en lugar de acabar, mostramos overlay y pasamos a RESCATE al pulsar continuar
          nextMode = "rescue";
          overlayWin.style.display="grid";
        }
      }
